...
const CSVN_commonEvents: PluginConsts = {
    params: PluginManagerEx.createParameter(document.currentScript),
    ipcRenderer: null,
};

const innFeeVarId: number = CSVN_commonEvents.params.innFeeVarId;
const aliveLeaderNameVarId: number =
    CSVN_commonEvents.params.aliveLeaderNameVarId;
const averageLevelVarId: number = CSVN_commonEvents.params.averageLevelVarId;

PluginManagerEx.registerCommand(
    document.currentScript,
    "setInnFee",
    (args: any) => {
        const innFee: number = args.innFee;
...
Game_Party.prototype.recoverWithoutState = function (this: Game_Party): void {
...
Game_Party.prototype.setAliveLeaderMemberId = function (
    ...
    const aliveLeader = this.battleMembers().find(
        (a: Game_Actor) => !a.isDeathStateAffected()
    );
    $gameVariables.setValue(aliveLeaderNameVarId, aliveLeader.actorId());
};

Game_Party.prototype.averageLevel = function (this: Game_Party): number {
    return (
