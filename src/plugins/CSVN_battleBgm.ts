...
import { BattleBgm } from "#/@types/plugins/CSVN_battleBgm";

const CSVN_battleBgm: PluginConsts = {
    params: PluginManagerEx.createParameter(document.currentScript),
    ipcRenderer: null,
};

// console.log(">>>>>>>> Hello from CSVN_battleBgm.", CSVN_battleBgm.params);

const battleBgmList: BattleBgm[] = CSVN_battleBgm.params.list;

//-----------------------------------------------------------------------------
// BattleManager

const _BattleManager_playBattleBgm_bbg = BattleManager.playBattleBgm;
BattleManager.playBattleBgm = function (this: BattleManager) {
    const troopId = $gameTroop._troopId;
    const battleBgm: BattleBgm = battleBgmList.find(
        (b) => b.troopId === troopId
    );
    if (battleBgm) {
        if (battleBgm.bgm) {
